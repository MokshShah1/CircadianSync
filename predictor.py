...
def load_and_predict_model(test_ratios, selected_option):
    ...
    if model_file_path is not None and os.path.exists(model_file_path):
        ...
        st.markdown('<style>' +
                    ...
                    '.main-content { flex: 1; background-color: #333333; border-radius: 10px; padding: 20px; margin-right: 20px; }' +
                    '.sidebar-content { width: 300px; background-color: #333333; border-radius: 10px; padding: 20px; }' +
                    '.centered-title { text-align: center; margin-bottom: 20px; }' +
                    '.predicted-scenario { font-weight: bold; color: #8A2BE2; font-size: 16px; margin-bottom: 10px; }' +
                    '.prediction-probabilities { font-style: italic; color: #8A2BE2; }' +
                    '.predict-button { background-color: #4CAF50; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; }' +
                    '.predict-button:hover { background-color: #45a049; }' +
                    '#pie-chart-container { width: 400px; margin-top: 20px; margin-left: auto; margin-right: 0; }' +
